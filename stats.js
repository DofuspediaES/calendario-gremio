(function(){
  "use strict";
  if (window.__guildStatsLoaded || window.__guildStatsLoading) return;
  window.__guildStatsLoading = true;
  ...
  async function load(){
    if(document.getElementById("guildStats")){window.__guildStatsLoaded=true;window.__guildStatsLoading=false;return;}
    styles();
    const {data:events,error:e1}=await db.from("events").select("id,name,type,event_date,event_time,capacity,attendance_confirmed");
    if(e1){console.error("Estadísticas:",e1);window.__guildStatsLoading=false;return;}
    const {data:parts,error:e2}=await db.from("event_participants").select("event_id,attended");
    if(e2){console.error("Estadísticas:",e2);window.__guildStatsLoading=false;return;}
    if(document.getElementById("guildStats")){window.__guildStatsLoaded=true;window.__guildStatsLoading=false;return;}
    const now=new Date();
    ...
    window.__guildStatsLoading=false;
  }
